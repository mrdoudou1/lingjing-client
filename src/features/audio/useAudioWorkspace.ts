...
import type { Asset, AudioRequest, ModelCapabilities, Notify } from '../../types/domain'
...
export function useAudioWorkspace(notify: Notify) {
  ...
  const [resultAsset, setResultAsset] = useState<Asset | undefined>()
  ...
  const submit = useCallback(async () => {
    ...
    setStatus('running'); setProgress(0); setResultAsset(undefined)
    try {
      const result = await audioJobs.start(input, state => { setProgress(state.progress); setStatus(state.status === 'succeeded' ? 'succeeded' : 'running') })
      if (result.status === 'succeeded') {
        await assetRepository.reload()
        const assets = await assetRepository.list()
        setResultAsset(assets.find(asset => asset.jobId === result.id))
        notify(tab === 'tts' ? '语音合成成功，已保存到素材库' : '语音识别成功，已保存到历史记录')
      }
    } catch { setStatus('failed'); notify('音频任务失败，请重试') }
  }, [notify, request, tab])
  const chooseFile = useCallback((file: File | undefined) => { if (file) { setSourceFile(file); notify(`已选择文件：${file.name}`) } }, [notify])
  return { tab, setTab: (next: 'tts' | 'stt') => { setTab(next); setStatus('idle'); setProgress(0); setResultAsset(undefined); setFormat(next === 'tts' ? 'MP3' : 'SRT') }, text, setText, model, setModel, models: modelState.models, modelsLoading: modelState.loading, voice, setVoice, language, setLanguage, format, setFormat, sourceFile, chooseFile, status, progress, resultAsset, submit }
}
