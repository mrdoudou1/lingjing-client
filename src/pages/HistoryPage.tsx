...
export function HistoryPage({ notify }: { notify: Notify }) {
  ...
  const virtual = useVirtualWindow(history.visible, 68, 6, 'history-virtual-scroll')
  return <div className="library-page"><div className="page-heading"><div><span className="eyebrow">活动记录</span><h1>历史记录</h1></div><button className="light-button" onClick={() => { void history.refresh(); notify('历史记录已刷新') }}>↻ 刷新</button></div><div className="filter-row">{(['全部','chat','image','video','tts','stt'] as const).map(item => <button key={item} className={history.filter === item ? 'selected' : ''} onClick={() => history.setFilter(item === '全部' ? '全部' : item)}>{item === '全部' ? '全部' : labels[item]}</button>)}</div>{history.loading ? <div className="history-placeholder"><span>◌</span><h2>正在加载历史</h2><p>读取本地任务记录。</p></div> : history.visible.length === 0 ? <div className="history-placeholder"><span>◷</span><h2>暂无历史记录</h2><p>完成聊天、图片、视频或音频任务后会显示在这里。</p></div> : <div id="history-virtual-scroll" className="history-list virtual-list"><div style={{ height: virtual.topSpacer }} aria-hidden="true" />{virtual.visibleItems.map(record => <button className="history-row" key={record.id} onClick={() => notify(`已打开 ${labels[record.kind]} 任务详情`)}><span className={`history-status ${record.status}`}>{record.status}</span><span><strong>{labels[record.kind]}任务</strong><small>{record.modelId || '未指定模型'} · {record.jobId.slice(0, 12)}</small></span><span><small>{new Date(record.createdAt).toLocaleString('zh-CN')}</small></span></button>)}<div style={{ height: virtual.bottomSpacer }} aria-hidden="true" /></div>}</div>
}
